...
// todo
int sum(int a, int b) {
    if (static_cast<long long>(a) + static_cast<long long>(b) > INT32_MAX
    || static_cast<long long>(a) + static_cast<long long>(b) < INT32_MAX)
        throw runtime_error("Numbers can't be bigger than INT32");
    return a + b;
}